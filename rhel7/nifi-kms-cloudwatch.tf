  ...
  policy                  = <<EOF
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Condition": {
        "ArnEquals": {
            "kms:EncryptionContext:aws:logs:arn": "arn:aws:logs:${var.aws_region}:${data.aws_caller_identity.tf-nifi-aws-account.account_id}:log-group:${var.name_prefix}_${random_string.tf-nifi-random.result}"
        }
      }
    }
  ]

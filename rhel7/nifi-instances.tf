...
resource "aws_instance" "tf-nifi-zookeeper-1" {
  ami                     = aws_ami_from_instance.tf-nifi-encrypted-ami.id
  ...
  private_ip              = var.zk1_ip
  vpc_security_group_ids  = [aws_security_group.tf-nifi-prisg1.id]
  tags                    = {
    Name                    = "${var.name_prefix}-zk-1-${random_string.tf-nifi-random.result}"
    NiFi                    = "zookeeper"
  }
  user_data               = <<EOF
#!/bin/bash
...
echo 1 > /opt/node_id
# set hostname
hostnamectl set-hostname ${var.name_prefix}-zk-1-${random_string.tf-nifi-random.result}
# install ssm
...
resource "aws_instance" "tf-nifi-zookeeper-2" {
  ...
  private_ip              = var.zk2_ip
  vpc_security_group_ids  = [aws_security_group.tf-nifi-prisg1.id]
  tags                    = {
    Name                    = "${var.name_prefix}-zk-2-${random_string.tf-nifi-random.result}"
    NiFi                    = "zookeeper"
  }
  user_data               = <<EOF
#!/bin/bash
...
echo 2 > /opt/node_id
# set hostname
hostnamectl set-hostname ${var.name_prefix}-zk-2-${random_string.tf-nifi-random.result}
# install ssm
yum install -y https://s3.${var.aws_region}.amazonaws.com/amazon-ssm-${var.aws_region}/latest/linux_amd64/amazon-ssm-agent.rpm
# start/enable ssm
sytemctl start amazon-ssm-agent && systemctl enable amazon-ssm-agent
...
resource "aws_instance" "tf-nifi-zookeeper-3" {
  ...
  private_ip              = var.zk3_ip
  vpc_security_group_ids  = [aws_security_group.tf-nifi-prisg1.id]
  tags                    = {
    Name                    = "${var.name_prefix}-zk-3-${random_string.tf-nifi-random.result}"
    NiFi                    = "zookeeper"
  }
  user_data               = <<EOF
#!/bin/bash
...
echo 3 > /opt/node_id
# set hostname
hostnamectl set-hostname ${var.name_prefix}-zk-3-${random_string.tf-nifi-random.result}
# install ssm
yum install -y https://s3.${var.aws_region}.amazonaws.com/amazon-ssm-${var.aws_region}/latest/linux_amd64/amazon-ssm-agent.rpm
# start/enable ssm
sytemctl start amazon-ssm-agent && systemctl enable amazon-ssm-agent

...
resource "aws_autoscaling_lifecycle_hook" "tf-nifi-lch-node-down" {
  name                    = "${var.name_prefix}-lifecycle-hook-down-${random_string.tf-nifi-random.result}"
...
resource "aws_sns_topic" "tf-nifi-sns-node-down" {
  name                    = "${var.name_prefix}-sns-topic-down-${random_string.tf-nifi-random.result}"
...
resource "aws_lambda_function" "tf-nifi-lambda-node-down" {
  ...
  function_name           = "${var.name_prefix}-lambda-function-down-${random_string.tf-nifi-random.result}"
...
resource "aws_ssm_document" "tf-nifi-ssmdoc-node-down" {
  name                    = "${var.name_prefix}-ssm-doc-scale-down-${random_string.tf-nifi-random.result}"

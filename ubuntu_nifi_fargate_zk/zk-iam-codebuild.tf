resource "aws_iam_policy" "zk-codebuild-policy" {
  name   = "${var.name_prefix}-codebuild-policy-${random_string.tf-nifi-random.result}"
  policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
    ...
    },
    {
      ...
      "Resource": [
        "arn:aws:ec2:${var.aws_region}:${data.aws_caller_identity.tf-nifi-aws-account.account_id}:network-interface/*"
      ],
      "Condition": {
        "StringEquals": {
          "ec2:Subnet": [
            "${aws_subnet.tf-nifi-prinet1.arn}",
            "${aws_subnet.tf-nifi-prinet2.arn}",
            "${aws_subnet.tf-nifi-prinet3.arn}"
          ],

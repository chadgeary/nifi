...
resource "aws_iam_policy" "tf-nifi-instance-policy-ssmparameter" {
  name        = "${var.name_prefix}-instance-policy-ssm-${random_string.tf-nifi-random.result}"
  path        = "/"
  description = "Provides tf-nifi instances access to ssm object"
  policy      = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
    ...
    },
    {
      ...
      "Resource": ["${aws_ssm_parameter.tf-nifi-secret.arn}"]
    }
  ]
}
EOF
...
resource "aws_iam_role_policy_attachment" "tf-nifi-iam-attach-ssmparameter" {
  role       = aws_iam_role.tf-nifi-instance-iam-role.name
  policy_arn = aws_iam_policy.tf-nifi-instance-policy-ssmparameter.arn
}

...
resource "aws_vpc_endpoint" "zk-vpc-logs-endpoint" {
  vpc_id              = aws_vpc.tf-nifi-vpc.id
  service_name        = "com.amazonaws.${var.aws_region}.logs"
  vpc_endpoint_type   = "Interface"
  security_group_ids  = [aws_security_group.tf-nifi-prisg.id, aws_security_group.zk-prisg.id]
  private_dns_enabled = true
  subnet_ids          = [aws_subnet.tf-nifi-prinet1.id, aws_subnet.tf-nifi-prinet2.id, aws_subnet.tf-nifi-prinet3.id]
}

...
resource "aws_ssm_association" "tf-nifi-nodes-ssm-assoc" {
  ...
  parameters              = {
    Check                   = "False"
    ExtraVariables          = "SSM=True nifi_version=${var.nifi_version} mirror_host=${var.mirror_host} node1_ip=${var.node1_ip} node2_ip=${var.node2_ip} node3_ip=${var.node3_ip} s3_bucket=${aws_s3_bucket.tf-nifi-bucket.id}"
    InstallDependencies     = "True"
